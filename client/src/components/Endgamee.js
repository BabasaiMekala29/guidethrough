...
function Endgame() {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            console.log("hiii")
            try {
                const response = await fetch(`https://guidethroughh.vercel.app/category/${head}/${subhead}`);
                if (!response.ok) {
    ...
    const handleClick = async (sec) => {
        ...
        setSelectedSec(sec);
        try {
            const response = await fetch(`https://guidethroughh.vercel.app/category/${head}/${subhead}/sortby/${sec}`);
            if (!response.ok) {
                throw new Error('Failed to fetch posts');
            }
