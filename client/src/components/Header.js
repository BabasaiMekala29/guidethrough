...
function Header() {
    ...
    useEffect(() => {
        const fetchNotificationCount = async () => {
            if (userid) {
                try {
                    const response = await fetch(`https://guidethroughh.vercel.app/user/${userid}/notifications_count`);
                    if (!response.ok) {
    ...
    useEffect(() => {
        const fetchTipNotification = async () => {
            if (userid) {
                try {
                    const response = await fetch(`https://guidethroughh.vercel.app/user/${userid}/tipNotification`);
                    if (!response.ok) {
                        throw new Error('Failed to fetch post notifications');
                    }
                    const data = await response.json();
    ...
    function logout() {
        fetch('https://guidethroughh.vercel.app/logout', {
            credentials: "include",
            method: "POST"
        })
            .then(() => {
    ...
    async function makeViewed(notif_id) {
        try {
            const response = await fetch(`https://guidethroughh.vercel.app/notifications/${notif_id}`, {
                method: 'PUT',
    ...
    const handleNotificationsClick = async (event) => {
        ...
        try {
            const response = await fetch(`https://guidethroughh.vercel.app/user/${userid}/notifications`);
            if (!response.ok) {
                throw new Error('Failed to fetch post notifications');
            }
            const data = await response.json();
